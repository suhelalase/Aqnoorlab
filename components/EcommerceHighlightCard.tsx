...
const highlights = [
  { icon: Sparkles, label: "Best Seller" },
  ...
  return (
    <div className="w-full max-w-lg mx-auto p-4">
      <div className="relative z-10">
        <Card className="group relative overflow-hidden rounded-[28px] border border-border/40 bg-background text-foreground shadow-xl">
          <div
            aria-hidden="true"
          ...
          <CardHeader className="relative space-y-4 pb-0">
            <Badge className="w-fit rounded-full bg-primary/15 px-3 py-1 text-[0.65rem] font-medium uppercase tracking-[0.25em] text-primary transition-colors hover:bg-primary hover:text-primary-foreground border-none">
              Nimbus Collection
            </Badge>
            <div className="space-y-2">
              ...
              <CardDescription className="text-sm text-muted-foreground">
                Lightweight performance kicks engineered for all-day comfort and momentum.
              </CardDescription>
            </div>
          ...
          <CardContent className="relative space-y-6 pt-6">
            <div className="flex flex-wrap gap-2">
              {highlights.map(({ icon: Icon, label }) => (
                <span
                  key={label}
                  className="inline-flex items-center gap-2 rounded-full border border-border/70 bg-muted/60 px-3 py-1.5 text-xs font-medium text-muted-foreground backdrop-blur"
                >
                ...
                {plans.map((plan) => {
                  ...
                  return (
                    <Button
                      ...
                      className={`rounded-full border-border/60 px-4 py-2 text-sm font-medium transition-all cursor-pointer ${
                        isActive
                          ? "border-primary/40 bg-primary text-primary-foreground shadow-lg hover:bg-primary/95 hover:text-primary-foreground"
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">{activePlan.title}</p>
                    <p className="text-base font-semibold text-foreground">{activePlan.price}</p>
                  </div>
                ...
                <p className="text-sm leading-relaxed text-muted-foreground">{activePlan.description}</p>
                <p className="text-xs font-medium text-foreground">{activePlan.promise}</p>
              </motion.div>
            </AnimatePresence>
          ...
          <CardFooter className="relative flex flex-col gap-4 border-t border-border/50 bg-muted/40 px-8 py-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm">
              <p className="font-medium text-foreground">Arrives before Friday</p>
              <p className="text-xs text-muted-foreground">Checkout before 2PM local time</p>
            </div>
            <Button size="lg" className="w-full rounded-full sm:w-auto font-semibold">
